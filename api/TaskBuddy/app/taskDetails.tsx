...
const API_URL = Platform.OS === 'android' 
  ? 'http://192.168.114.166:8080/api/tasks' 
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            task.subTasks.map((sub: any, index: number) => (
              ...
              >
                <Ionicons 
                  ...
                  size={24} 
                  color={sub.isCompleted ? colors.succes : colors.subText} 
                />
                <Text style={[
                  styles.subtaskText, 
                  { color: colors.text },
      ...
      <View style={[styles.footer, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        >
          {saving ? (
            <ActivityIndicator color={task.status === 'Completed' ? colors.primary : "#FFF"} />
          ) : (
            <Text style={[
              styles.buttonText, 
              task.status === 'Completed' ? { color: colors.primary } : { color: "#FFF" }
            ]}>
